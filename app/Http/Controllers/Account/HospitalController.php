...
class HospitalController extends BaseController
{
    ...
    {
        $this->_setPageTitle('Hospital Profile');
        $data = [
            'title' => 'Hospital Profile',
            'user' => Auth::user(),
        ];

        return view("account.profiles.hospital.profile")->with($data);
    }
    ...
    public function showProfile(Request $request)   {
        ...
        try {
            $user = Auth::user();
            $documents = $user->hospitalDocuments;
            $data = ['title' => 'Hospital Profile', 'user' => $user, 'documents' => $documents];
            if($request->type && $request->type == 'approved-document')
                $html = view('account.profiles.hospital.verified_hospital_document_modal', $data)->render();
            else
                $html = view('account.profiles.hospital.document_upload', $data)->render();
    ...
    public function storeProfile(Request $request)  {
        ...
        if ($validator->fails()) {
            ...
            try {
                ...
                $user->update(['is_hospital_verified' => '1']);
                ...
                $data = [
                    ...
                    'title' => 'User Request For Hospital Profile Approval',
                    'type' => 'hospital_profile_verification',
                    'message' => Auth::user()->name . ' has requested for approval profile as a pharmacy. please verify the details.',
                ];
                ...
                if ($is_admin->email) {
                    $mailInfo = ([
                        ...
                        'title' => '',
                        'subject' => 'Apply Profile As Hospital ' . $user->name,
                        'content' => 'I would like to request approval for my profile as a hospital. I have uploaded all the details and documents.<br>
