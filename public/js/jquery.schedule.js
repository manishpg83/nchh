  ...
  $.extend(Plugin.prototype, {
    ...
    create: function () {
      ...
      for (var k = 0; k < this.settings.days; k++) {
        ...
        // customized this condition (remove all)
        $('<div class="jqs-grid-day">' + this.settings.daysList[k] + '</div>').appendTo($('.jqs-grid-head', this.element));
      }
    },
    ...
    add: function (parent, position, height, options) {
      ...
      var periodRemove = '';
      var periodDuplicate = '';
      if (this.settings.mode === 'edit' && !options.is_exist) {
      ...
      var is_exist = options.is_exist ? 'allocated' : '';
      ...
      var period = $('<div class="jqs-period">' +
        '<div class="jqs-period-container ' + is_exist + '">' + periodTime + periodTitle + periodRemove + periodDuplicate + '</div>' +
        '</div>').css({
          'top': position * this.periodPosition,
          'height': height * this.periodPosition
    ...
    removeAll: function (parent) {
      ...
      $('.jqs-period', parent).each(function (index, period) {
        console.log("call remove all function");
        console.log(period);
        if (!period.is_exist) {
          $this.remove(period);
        }
    ...
    export: function () {
      ...
      $('.jqs-day', $this.element).each(function (index, day) {
        ...
        $('.jqs-period', day).each(function (index, period) {
          if ($(period).find('.allocated').length == 0) {
