...
$document.ready(function() {
    ...
    hospitalProfileModal.on("shown.bs.modal", function() {
        validateDocName();
    });
});

function loadHospitalProfileVerificationModal() {
    if (typeof uploadDocumentFormURL !== "undefined") {
        $.ajax({
            ...
            success: function(res) {
                hospitalProfileModal.html(res.html);
                hospitalProfileModal.modal({
                    show: true,
                    backdrop: "static",
                    keyboard: false
                });
...
function viewHospitalverifieddocument() {
    if (typeof uploadDocumentFormURL !== "undefined") {
        $.ajax({
            url: uploadDocumentFormURL,
            type: "GET",
...
function init_documentForm() {
    hospitalProfileForm = $document.find("#hospitalProfileForm");
    //Jquery validation of form field
    hospitalProfileForm.validate({
        rules: {
            'document_name[*]' : {
                required: true
            },
        ...
        submitHandler: function(form) {
            ...
            $.ajax({
                ...
                beforeSend: function() {
                    ...
                    hospitalProfileForm.find(".close-button").addClass("disabled");
                },
                success: function(data) {
                    if (data.status == 200) {
                        hospitalProfileModal.modal("toggle");
                        hospitalProfileForm.trigger("reset");
